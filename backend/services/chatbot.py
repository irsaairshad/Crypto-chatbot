...
def get_gemini_credentials():
    # Search all possible .env locations and reload
    possible_paths = [
        Path(__file__).resolve().parents[2] / ".env",
        Path(__file__).resolve().parents[1] / ".env",
        Path(__file__).resolve().parents[3] / ".env",
        Path.cwd() / ".env",
        Path.cwd() / "crypto-chatbot-main" / ".env",
    ]
    ...
    if not key or not key.strip():
        ...
        key = "AQ.Ab8RN6LxobJA3GWBnyFxmiBx1XvaXz5qngmRSdW3C7D5N9atnA"
...
SYSTEM_PROMPT = """
ROLE
You are CryptoVision AI Assistant, a professional AI assistant specializing in
...
"""
    ...
    if not api_key:
        # Fallback to direct live data formatting if GEMINI_API_KEY is not provided
        crypto_data = live_data.get("cryptocurrencies") or []
        fiat_data = live_data.get("fiat_conversion")
        
        if crypto_data:
            lines = ["Here is the verified live market data:"]
            for c in crypto_data:
                if "error" in c:
                    lines.append(f"• {c.get('name', 'Unknown')}: {c['error']}")
                else:
                    change_str = f" ({c['change_24h_percent']:+.2f}% 24h)" if c.get("change_24h_percent") is not None else ""
                    lines.append(f"• **{c['name']} ({c['symbol']})**: ${c['price']:,.2f}{change_str} | Market Cap: ${c.get('market_cap', 0):,.0f} | 24h Vol: ${c.get('volume_24h', 0):,.0f}")
            lines.append("\n*To enable conversational analysis, add GEMINI_API_KEY in your .env file.*")
            return {"reply": "\n".join(lines), "response_id": f"live-{uuid.uuid4()}"}
        
        if fiat_data and "error" not in fiat_data:
            return {
                "reply": f"**Currency Conversion**: {fiat_data.get('amount', 1)} {fiat_data.get('from_currency')} = {fiat_data.get('converted_amount'):,.2f} {fiat_data.get('to_currency')} (Rate: {fiat_data.get('exchange_rate'):,.4f})",
                "response_id": f"fiat-{uuid.uuid4()}"
            }
            
        return {
            "reply": "CryptoVision AI is connected. To enable full AI chat and market insights, please add your `GEMINI_API_KEY` to the `.env` file in the project directory.",
            "response_id": f"system-{uuid.uuid4()}"
        }
